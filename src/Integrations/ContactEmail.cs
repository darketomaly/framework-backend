...
public static class ContactEmail
{
    ...
    {
        app.MapPost("/contact", async (HttpRequest request, IHttpClientFactory httpClientFactory, ILogger<Program> logger) =>
        {
            if (!request.HasFormContentType)
            {
                return Results.BadRequest(new { error = "The request must use multipart/form-data." });
            }
            ...
            if (!IsValidEmail(email) || string.IsNullOrWhiteSpace(message) || message.Length > 10_000)
            {
            ...
            {
                logger.LogError("Contact email configuration is incomplete.");
            ...
            var html = BuildHtml(displayName, email, message);
            var client = httpClientFactory.CreateClient();
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", apiKey);

            var response = await client.PostAsJsonAsync("https://api.resend.com/emails", new ResendEmail(
                sender,
                [recipient],
            ...
            {
                logger.LogError("Resend rejected a contact email with status {StatusCode}.", response.StatusCode);
                  ...
                  <tr><td align="center">
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#101416;border:1px solid #38434a;border-radius:14px;padding:32px;">
                      <tr><td>
                        ...
                        <div style="border-left:3px solid #b8c1c8;padding:4px 0 4px 16px;color:#eaf0f2;line-height:1.6;white-space:pre-wrap;">{Escape(message)}</div>
                      </td></tr>
                    </table>
                  </td></tr>
                </table>
